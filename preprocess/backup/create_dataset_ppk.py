...
def main():
    ...
    num_step    = -1 + win_size*100 /step_stride
    
    stream_paths = '/data/WC_AItrain/Man_30s/All/Events/*Z.SAC'
    stream_dir   = '/data/WC_AItrain/Man_30s/All/Events/'
    output_dir   = '/home/zhouyj/Documents/CONEDEP/data/train/WC_man30s/all/ppk_test'
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)
    stream_files = sorted(glob.glob(stream_paths))
    
    # feature extractor
    ckpt_path = '/home/zhouyj/Documents/CONEDEP/output/NPS_depth5/NPSNet' #TODO
    sess = tf.Session()
    raw_inp = {'data': tf.placeholder(tf.float32,
                                      shape=(1, 1, step_len+1, 3),
                                      name='raw_inp')}
    NPSNet = models.get('NPSNet', raw_inp, cfg, ckpt_path, sess)
    NPSNet.load(sess, None)
    to_fetch = NPSNet.layers['conv5'] # the output of CNN
    width, kernel = to_fetch.shape[1], to_fetch.shape[2]
    print 'prep/create_ppk to_fetch', to_fetch
    ...
    for stream_file in stream_files:
        ...
        for i in range(len(stx_paths)):
            ...
            if (n_traces == 3) and (n_pts == n_samples):
                # def input of RNN
                input_array = np.zeros((num_step, kernel, width), dtype=np.float32)
                # three chn data
                xdata = stream[0].data
                ydata = stream[1].data
                zdata = stream[2].data
                ...
                for j in range(num_step):
                    ...
                    current_step = np.array([[np.transpose(st_data[:, idx_s:idx_e])]])
                    feed_dict = {raw_inp['data']: current_step}
                    # run NPSNet
                    current_step = sess.run(to_fetch, feed_dict)
                    input_array[j, :, :] = np.transpose(current_step[0]) #TODO
